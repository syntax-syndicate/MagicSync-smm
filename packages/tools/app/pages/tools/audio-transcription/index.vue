...
import { useTranscription, type ModelKey } from '~/composables/useTranscription'
					...
					<BaseShinyCard class="w-full">
						<UCard class="p-6">
							<div class="flex flex-wrap items-end gap-4 mb-6 pb-6 border-b border-border">
								...
								<div v-if="isModelLoaded" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-500 text-sm">
						...
						<UCard class="p-6">
							...
							<div v-if="selectedFile?.result" class="space-y-4">
								<UTabs :items="[
									...
									{ label: 'Timestamps', value: 'timestamps', icon: 'i-lucide-clock' }
								]">
									<template #default="{ item }">
										<div v-if="item.value === 'text'" class="prose prose-sm max-w-none">
											<p class="whitespace-pre-wrap leading-relaxed">{{ selectedFile.result?.text }}</p>
										</div>
										<div v-else-if="item.value === 'json'" class="space-y-3">
